...
export default function NuevoCorteDialog() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!form.tasaBinance) {
      toast.error('Ingresa la tasa Binance');
      return;
    }
    if (!form.usdtRecibidoReal) {
      toast.error('Ingresa el USDT recibido');
      return;
    }
        ...
        <form noValidate onSubmit={handleSubmit} className="flex flex-col flex-1 min-h-0">
          ...
          <div className="sticky bottom-0 bg-background pt-4 pb-1 border-t border-border mt-4">
            <Button
              type="submit"
              className="w-full"
              disabled={submitting || !form.pais || !form.tasaBinance || !form.usdtRecibidoReal}
            >
              {submitting ? 'Registrando...' : `Registrar Corte${parseFloat(form.usdtRecibidoReal) > 0 ? ` — ${form.usdtRecibidoReal} USDT` : ''}`}
            </Button>
          </div>
        </form>
      </DialogContent>
